...
#else: 
#	filename = sys.argv[1]
#	plotname = os.path.splitext(filename)[0] + ".pdf"
...
def getGpu():
    ...
    text = runShellCommand(command).split('\n')
    product = [ line for line in text if re.search("product", line) ][0].split("product:")[1].strip()
    vendor = [ line for line in text if re.search("vendor", line) ][0].split("vendor:")[1].strip()
    driver = [ line for line in text if re.search("driver", line) ][0].split("configuration:")[1].strip().split(" ")[0].split("=")[1].strip()
...
def getDAQ():
    ...
    try: 
       daqline = daqline[0]
       return daqline.split(":")[2].strip() # FIXME
    except
       print("no daq found")
       return ""
...
def getChannelIndices(f, n):
    ...
    ct_idx =  [ headers.index(channel) for channel in headers if re.match(r".*Performance Measurement.*Comp Time.*", channel) ][0]
    pe_idx =  [ headers.index(channel) for channel in headers if re.match(r".*Performance Measurement.*Real-time Period.*", channel) ][0]
    jt_idx =  [ headers.index(channel) for channel in headers if re.match(r".*Performance Measurement.*Jitter.*", channel) ][0]
...
filename="performance.h5"
plotname="performance.png"
#plotname="performance.pdf"
trialnum=1
...
#plotname = os.path.splitext(filename)[0] + "_trial" + str(trialnum) + ".pdf"

period = getPeriod(hdf, trialnum) / 1e6 #  ns -> ms
frequency = 1/period
downsampling = getDownsamplingRate(hdf, trialnum)
#length = getTrialLength(hdf, trialnum) / 1e9 #  ns -> s
...
frame = frame.rename(index=str, columns={"Comp Time (ns)":"Comp Time (us)", "Real-time Period (ns)":"Real-time Period (us)", "RT Jitter (ns)":"RT Jitter (us)"})
...
f, ax = plt.subplots(3, sharex=True, figsize=(11.5,8), dpi=80)
ax[0].scatter(x=frame['Time (s)'], y=frame['Comp Time (us)'], alpha=.2, marker=".", edgecolor='none')
#ax[0].spines['top'].set_visible(False)
#ax[0].spines['right'].set_visible(False)
ax[0].set_ylabel("Comp Time (us)")
ax[1].scatter(x=frame['Time (s)'], y=frame['Real-time Period (us)'], alpha=.2, marker=".", edgecolor='none')
#ax[1].spines['top'].set_visible(False)
#ax[1].spines['right'].set_visible(False)
ax[1].set_ylabel("Real-time Period (us)")
ax[2].scatter(x=frame['Time (s)'], y=frame['RT Jitter (us)'], alpha=.2, marker=".", edgecolor='none')
#ax[2].spines['top'].set_visible(False)
#ax[2].spines['right'].set_visible(False)
ax[2].set_ylabel("RT Jitter (us)")
